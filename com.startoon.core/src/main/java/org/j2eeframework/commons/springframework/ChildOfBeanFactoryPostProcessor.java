package org.j2eeframework.commons.springframework;
...
/**
 * 解释使用注解方式注入Bean的parent问题
 * •BeanFactoryPostProcessor for Inheritance of Bean Properties via Annotations 
 * •{@link at.gv.brz.spring.beans.ChildOf * @ChildOf} • 
 * •@author HAUSER 
 * •@since 2009-05-12 •
 */
public class ChildOfBeanFactoryPostProcessor implements BeanFactoryPostProcessor, PriorityOrdered {
	 ...
	 * •properties even to eager-initializing beans. • •@param beanFactory •the bean factory used by the application
	 * context •@throws org.springframework.beans.BeansException •in case of errors
	 */
	...
	public void postProcessBeanFactory(ConfigurableListableBeanFactory beanFactory) throws BeansException {
		...
		for (int i = 0; i < beanNames.length; i++) {
			...
			try {
				...
				ChildOf childOf = AnnotationUtils.findAnnotation(clazz, ChildOf.class);
				if (childOf != null) {
					...
					if (logger.isDebugEnabled()) {
						logger.debug("Found parentName [" + parentName + "] for bean [" + beanName + "]");
					}
				...
				logger.error("Bean [" + beanName + "] has invalid ClassName[" + bd.getBeanClassName() + "] - Exception will be ignored");
			}
		}
