 ...
 * instance between the {@link StartoonConnectorServlet connector servlet} and the
...
public class MyDispatcher {
	 ...
	 * Called at connector servlet {@link StartoonConnectorServlet#init()
	 * initialization}.
	 * 
	 ...
	 */
	MyDispatcher(final ServletContext servletContext) throws Exception {
	 ...
	 *             due to some reason
	 */
	private GetResponse getFoldersAndOrFiles(final Command command, final ResourceType type, final String currentFolder, final String constructedUrl) throws InvalidCurrentFolderException, ReadException {
	...
	UploadResponse doPost(final HttpServletRequest request) {
		...
		// check parameters  
			...
			try {
				...
				fileName = UUID.randomUUID().toString().replace("-", "") + "." + FilenameUtils.getExtension(fileName);
