...
public class PostedProAction extends ServiceBaseManageAction<PostedPro, Long> {
	...
	private static final long serialVersionUID = 8876090591859415756L;
	...
	public String insert() {
		 ...
		 * 设置直接类别
		 */
		if (categoryId4 != null && !"".equals(categoryId4)
				&& !"null".equals(categoryId4)) {
		 ...
		 * 设置类别属性字符串
		 */
		String nameStr = getSplitKey(categoryName2)
				+ getSplitKey(categoryName2) + getSplitKey(categoryName3)
				+ getSplitKey(categoryName4);
		String idStr = getSplitKey(categoryId1) + getSplitKey(categoryId2)
				+ getSplitKey(categoryId3) + getSplitKey(categoryId4);
