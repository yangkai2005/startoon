...
@Repository
public class CertDAOImpl extends GenericHibernateDAO<Cert, Long> implements ICertDAO {
	@Override
	public int getCountOfEntitiesByParamCondition(ParamCondition paramCondition) {
		IQueryParameterParser parser = new CertQueryParameterParser();
	...
	@Override
	public List<Cert> getEntitiesByParamCondition(ParamCondition paramCondition, int firstResult, int pageSize) {
		IQueryParameterParser parser = new CertQueryParameterParser();
		List<Criterion> criterions = parser.getCriterions(paramCondition);
		List<Order> orders = parser.getOrder(paramCondition);
		List<Cert> entitys = findEntityByCriteria(criterions, firstResult, pageSize, orders);
		return entitys;
	}

}

class CertQueryParameterParser implements IQueryParameterParser {
