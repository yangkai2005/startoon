...
public class DefaultAction extends ActionSupport {
	...
	private static final long serialVersionUID = -7443149352972573640L;
	...
	private final List<Info> focus = new ArrayList<Info>();
	...
	public String execute() {
		...
		pager.getParamCondition().addParameter("hot", "true");
		String[] focusInfoTypeIds = new String[] { "13", "14", "15", "16", "17", "18" };
		pager.getParamCondition().addParameterValues("infoTypeIds", focusInfoTypeIds);
		pager.setPageSize(6);
		infoService.getEntitiesOfPagerByParamCondition(pager);
		focus.addAll(pager.getItems());
		// 焦点 end
		...
		imgInfos = infoService.getInfoByInfoType(18L, 7); // 图片
		...
		copartnerships = copartnershipService.getCopartnershipByInfoTypeId(1L, 7);// 合作单位
		...
		barLatestInfos = infoService.getInfoByInfoTypes(barInfoTypeIds, 4);

		barTopInfos = infoService.getTopInfo(4);
		...
		// 人才服务
		info45 = infoService.getHrServiceInfo(45L, 4); // 人才工作站
		jobs = jobsService.getLatestJobsBySize(4); // 企业招聘
		talents = talentService.getLatestTalent(4); // 人才服务

		infoType = new InfoType();
		infoType.setId(1L);

		// 友情链接
		links = linkService.getLinksByType(2L, 10);
