...
// [更新日期] 2009-03-03
// [版 本 号] ver3.5  modified by truly 2010.02.03
//====================================================================================================
...
//			showmsg = $.formValidator.isEmpty(id) ? setting.onempty : setting.oncorrect;
			showmsg = $.formValidator.isEmpty(id) ? '' : setting.oncorrect;
    ...
    getLength : function(id)
    {
        ...
        switch(sType)
		{
			...
			case "textarea":
			case "file":
		        /* hack 将双字节转换成单字节,用于验证中英文混排字符串长度  */
		         var val = srcjo.val();
		        if(val)
		        {
				...
				if(returnObj)
				{
					...
					if (!initConfig.alertmessage){
						if (error_tip.indexOf("^" + tipid + "^") == -1) {
							...
							elem.focusforpagevalid=true;// truly
		...
		if(srcTag == "input" || srcTag=="textarea")
		{
			...
			{	
				if(!initConfig.alertmessage){
					//保存原来的状态
					var tipjq = $("#"+tip);
					if(this.focusforpagevalid)  return;// add by truly
			...
			jqobj.bind(setting.triggerevent, function(){
			    this.focusforpagevalid = false;// add by truly
